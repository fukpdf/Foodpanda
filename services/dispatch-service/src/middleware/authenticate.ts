...
async function getPublicKey(): Promise<Awaited<ReturnType<typeof importSPKI>>> {
  const configured = env.AUTH_PUBLIC_KEY_BASE64 ? Buffer.from(env.AUTH_PUBLIC_KEY_BASE64, "base64").toString("utf8") : null;
  if (_publicKey && _publicKeyPem === configured) return _publicKey;
  if (configured) {
    _publicKeyPem = configured;
    _publicKey = await importSPKI(configured, "RS256");
    return _publicKey;
  }
  const response = await fetch(`${env.AUTH_SERVICE_URL}/auth/public-key`, { signal: AbortSignal.timeout(5_000), headers: { accept: "application/json" } });
...
export async function authenticate(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  ...
  try {
    const { payload } = await jwtVerify(token, await getPublicKey(), { issuer: env.JWT_ISSUER, audience: env.JWT_AUDIENCE, algorithms: ["RS256"] });
    const claims = AuthClaimsSchema.parse({ sub: payload.sub, sessionId: payload["sessionId"], role: payload["role"], email: payload["email"] });
