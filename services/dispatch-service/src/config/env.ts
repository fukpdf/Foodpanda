...
const DispatchEnvSchema = z.object({
  ...
  ORDER_SERVICE_URL: z.string().url().default("http://localhost:3014"),
  ORDER_SERVICE_INTERNAL_KEY: productionSecret(32),
  DISPATCH_SERVICE_INTERNAL_KEY: productionSecret(32),
  ...
  REALTIME_INTERNAL_KEY: productionSecret(32),
  DISPATCH_SWEEP_INTERVAL_MS: z.coerce.number().int().min(5_000).default(30_000),
});

export type DispatchEnv = z.infer<typeof DispatchEnvSchema>;
export interface ResolvedDispatchEnv extends DispatchEnv { isDev: boolean; isProd: boolean; allowedOrigins: string[]; }
...
export function loadEnv(): ResolvedDispatchEnv {
  ...
  if (!result.success) throw new Error(`[dispatch-service] Environment validation failed:\n${result.error.issues.map((i) => `  ${i.path.join(".")}: ${i.message}`).join("\n")}`);
  const raw = result.data;
  _env = { ...raw, isDev: raw.NODE_ENV !== "production", isProd: raw.NODE_ENV === "production", allowedOrigins: raw.ALLOWED_ORIGINS.split(",").map((o) => o.trim()).filter(Boolean) };
  return _env;
}
export const env = new Proxy({} as ResolvedDispatchEnv, { get(_target, prop) { return loadEnv()[prop as keyof ResolvedDispatchEnv]; } });
